...
*/
	class Nepali_Calendar {
		...
		private function get_day_of_week_in_nepali($day){
			switch($day){
				case 1:
					$day = "आइतबार";
					break;
		...
		public function AD_to_BS($yy,$mm,$dd){
			if ($this->is_range_eng($yy, $mm, $dd) == false){
				...
				$this->nep_date["year"] =$this->convert_into_nepali_date($y); ;
				$this->nep_date["month"] = $this->convert_into_nepali_number($m);
				$this->nep_date["date"] =$this->convert_into_nepali_date($total_nDays);
				$this->nep_date["day"] = $this->get_day_of_week_in_nepali($day);
		...
		public function convert_into_nepali_number($englishNumber){
			$nepaliNumber = false;
		...
		public function convert_into_nepali_date($englishDate){
			$nepaliDate="";

			for($i=0;$i < strlen($englishDate);$i++){
				$englishNumber=substr($englishDate,$i,1);

				$nepaliDate=$nepaliDate.$this->convert_into_nepali_number($englishNumber);
			}

			return $nepaliDate;
		}

		public function get_time($desired_date){
			date_default_timezone_set('Asia/Kathmandu');

			$to_time = strtotime(date("Y-m-d h:i:s a"));
			$from_time=strtotime($desired_date);
			$mins=round(abs($to_time - $from_time)/60,1);

			$roundhour=round($mins/60);
			$hour=$mins/60;

			if($roundhour >23){
				if($roundhour<47){
					$hours=$this->convert_into_nepali_date(date("h",$from_time));
					$min=$this->convert_into_nepali_date(date("i",$from_time));
					$am_pm=date("A",$from_time);
					$result="हिजो ".$hours.":".$min." ".$am_pm;
				}else{
					$year=date("Y",$from_time);
					$month=date("m",$from_time);
					$date=date("j",$from_time);
					$nepalidate=$this->AD_to_BS($year,$month,$date);
					$hour=$this->convert_into_nepali_date(date("h",$from_time));
					$min=$this->convert_into_nepali_date(date("i",$from_time));
					$am_pm=date("A",$from_time);
					$result=$nepalidate['year'].' '.$nepalidate['nmonth'].' '.$nepalidate['date'].', '.$nepalidate['day']." ".$hour.":".$min." ".$am_pm." &nbsp;";
				}
			}else{
				if($hour<$roundhour){
					$roundrslt=$roundhour-1;
					if($hour >=1){
						$result=($roundhour==1 || $roundrslt==2)?"१ घण्टा अगाडी ":" ".$this->convert_into_nepali_date($roundrslt)." घण्टा अगाडी ";
					}else{
						
						$result=($mins==1)?" १ मिनेट अगाडी":" ".$this->convert_into_nepali_date(round($mins))." मिनेट अगाडी";
					}
				}else{
					if($hour >=1){
						$result=($roundhour==1)?"१ घण्टा अगाडी ":" ".$this->convert_into_nepali_date($roundhour)." घण्टा अगाडी";
					}else{
						$result=($mins==1)?"१ मिनेट अगाडी ":" ".$this->convert_into_nepali_date(round($mins))." मिनेट अगाडी ";
					}
				}
