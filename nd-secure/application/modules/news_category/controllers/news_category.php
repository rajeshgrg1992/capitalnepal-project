...
class News_category extends MX_Controller{
    ...
    public function category($id=0)
    {
        if($this->input->post())
        {
            ...
            $folder_path = '../uploads/news_category/';
            ...
            $url= strtolower(preg_replace('/[^A-Za-z0-9\-]/', '-', $this->input->post('category_url'))) ;
            ...
            if($category_id == "")
            {
                ...
                if($result)
                {
                    ...
                    $this->session->set_flashdata('success','News category Information has been added.');
                    redirect($this->redirect);
                }
                else{
                    $this->session->set_flashdata('error','Unable to add the news category Information.');
                    redirect($this->redirect);
                }
            ...
            if($result)
            {
                ...
                $log['action_id']= "2";
                $this->category_create_log($log);
                $this->session->set_flashdata('success','News Category Information has been updated.');
                redirect($this->redirect);
            }
            else{
                $this->session->set_flashdata('error','Unable to update the news category Information.');
                redirect($this->redirect);
            }
        ...
        $data['title']= "Add/Edit News Category";
        $this->load->view('header', $data);
    ...
    public function delete_category($category_id)
    {
        ...
        if($result)
        {
            ...
            $log['action_id']= "3";
            $this->category_create_log($log);
            $this->session->set_flashdata('success','News Category has been deleted.');
            redirect('news_category');
        }
        else{
            $this->session->set_flashdata('error','Unable to delete the news Category.');
            redirect('news_category');
        }
...
//            exit;
            ...
            if($remove) {
                ...
                if ($result) {
                    ...
                    $this->session->set_flashdata('success', 'Related News has been added.');
                    redirect('news_category');
                } else {
                    $this->session->set_flashdata('error', 'Unable to add related news.');
                    redirect('news_category');
                }
            }
            }
        ...
        $data['title']= "Add/Edit Related News";
        $this->load->view('header', $data);
    ...
    public function category_create_log($insert)
    {
        ...
        $insert['module_name'] =  "News Category";
