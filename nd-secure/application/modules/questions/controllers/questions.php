...
class Questions extends MX_Controller{
    public function __Construct()
    {
        ...
        $this->load->model('questions_model','content');
           ...
           // $insert['parent_id'] = $this->input->post('parent_id');
            ...
            if($insert['content_type'] == "Page" || "Services")
            {
                ...
                if($result)
                {
                    ...
                    $this->session->set_flashdata('success','New question has been inserted.');
                    redirect('questions');
                }
                else{
                    $this->session->set_flashdata('error','Unable to insert the new question.');
                    redirect('questions');
                }
            }
            else{
...
//                    exit;
                    ...
                    if($result)
                    {
                        ...
                        $log['action_id']= "2";
                        $this->create_log($log);
                        $this->session->set_flashdata('success','Question has been updated.');
                        redirect('questions');
                    }
                    else{
                        $this->session->set_flashdata('error','Unable to update the question.');
                        redirect('questions');
                    }
                }
                else
