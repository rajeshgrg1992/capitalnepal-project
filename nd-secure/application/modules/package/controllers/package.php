...
//            $num2 = rand(1111111111,9999999999);
            ...
            $insert['package_url'] = strtolower(str_replace(" ","-",$insert['package_name']));

            $insert['brand_id'] = $this->input->post('brand_id');
            ...
            if($package_id == "0")
            {
                ...
                if($result)
                {
                    ...
                    if($result)
                    {
                        $this->session->set_flashdata('success','New Product is Added Successfully');
                        redirect('package');
                    }
                    else{
        ...
        $data['brands'] = $this->crud->get_active_records('igc_clients');
    ...
    public function check_name_exist($str, $package_id)
    {
        ...
        {
            $this->form_validation->set_message('check_name_exist', 'Product name already exist.');
            return FALSE;
        }
    ...
    public function delete_package($package_id)
    {
        ...
        if($result)
        {
            ...
            $this->session->set_flashdata('error','Unable to delete Product');
            redirect('package');
        }
    ...
    public function gallery($package_id)
    {
        ...
        if($this->input->post())
        {
            ...
            if($result)
            {
                $this->session->set_flashdata('success','Product Gallery has been Updated.');
                redirect('package');
            }
            else
            {
    ...
    public function destination($package_id)
    {
        ...
        if($this->input->post())
        {
            ...
            if($result)
            {
                $this->session->set_flashdata('success','Product Destination has been Updated.');
                redirect('package');
            }
            else
            {
