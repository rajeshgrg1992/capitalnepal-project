...
class Package_category extends MX_Controller{
    ...
    public function category($id=0)
    {
        if($this->input->post())
        {
            ...
            if($category_id == "")
            {
                ...
                if($result)
                {
                    ...
                    $this->session->set_flashdata('success','Product category Information has been added.');
                    redirect($this->redirect);
                }
                else{
                    $this->session->set_flashdata('error','Unable to add the Product category Information.');
                    redirect($this->redirect);
                }
            ...
            if($result)
            {
                ...
                $log['action_id']= "2";
                $this->category_create_log($log);
                $this->session->set_flashdata('success','Product Category Information has been updated.');
                redirect($this->redirect);
            }
            else{
        ...
        $data['title']= "Add/Edit Product Category";
        $this->load->view('header', $data);
    ...
    public function delete_category($category_id)
    {
        ...
        if($result)
        {
            ...
            $log['action_id']= "3";
            $this->category_create_log($log);
            $this->session->set_flashdata('success','Product Category has been deleted.');
            redirect('package_category');
        }
        else{
            $this->session->set_flashdata('error','Unable to delete the Product Category.');
            redirect('package_category');
        }
...
//            exit;
            ...
            if($remove) {
                ...
                if ($result) {
                    ...
                    $this->session->set_flashdata('success', 'Related Product has been added.');
                    redirect('package_category');
                } else {
        ...
        $data['title']= "Add/Edit Related Product";
        $this->load->view('header', $data);
    ...
    public function category_create_log($insert)
    {
        ...
        $insert['module_name'] =  "Product Category";
