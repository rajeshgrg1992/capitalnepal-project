...
class Mainland extends MX_Controller{
    public function __Construct()
    {
        ...
        $this->load->model('mainland_model','content');
    ...
    public function index()
    {
        ...
        $data['title']= "Mainland List";
        $this->load->view('header', $data);
        $this->load->view('mainland_list');
    ...
    public function form($id=0)
    {
        ...
        if($this->input->post())
        {
            ...
            $this->form_validation->set_rules('freezone_page_type', 'freezone_page_type');
...
$insert['freezone_page_type'] = $this->input->post('freezone_page_type');
           ...
           // $insert['parent_id'] = $this->input->post('parent_id');
            ...
            $freezoneimg= $rand. str_replace(" ","",$_FILES['freezone_img']['name']);
            $freezoneimgtmp=$_FILES['freezone_img']['tmp_name'];
            ...
            if($content_id =="")
            {
                ...
                if($_FILES['freezone_img']['name'] !="")
                {
                    $insert['freezone_img']= $freezoneimg;

                        move_uploaded_file($freezoneimgtmp,$folder_path.$freezoneimg);
...
if($_FILES['freezone_img']['name'] !="")
                {
                    $pre_freezone_img = $this->input->post('pre_freezoneimg');

                    @unlink($folder_path.$pre_freezone_img);
...
//                    exit;
        ...
        $data['title']= "Add/Edit Mainland";
        $this->load->view('header', $data);
        $this->load->view('mainland_form');
        $this->load->view('footer');
    }
    ...
    public function delete($content_id)
    {
        ...
        if($result)
        {
            ...
            @unlink($path.$content_detail['featured_img']);
             @unlink($path.$content_detail['freezone_img']);
