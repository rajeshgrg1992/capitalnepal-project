...
//function to create log
    ...
    public function currency_form($id=0){
        $this->load->library('form_validation');
        if($this->input->post()){
            $this->form_validation->set_rules('currency', 'Currency', 'required');
            $this->form_validation->set_rules('currency_code', 'Currency Code', 'required');
            $this->form_validation->set_rules('publish_status', 'Publish Status');
            if ($this->form_validation->run())
            {
                ...
                $insert['publish_status']=(!empty($publish) && $publish=="on")?'1':'0';
                $result=($id > 0)?$this->crud->update($id,'currency_id',$insert,'igc_forex_currency_detail'):$this->crud->insert($insert,'igc_forex_currency_detail');

                if($result){
                    $this->session->set_flashdata('success',($id > 0)?'Currency has been updated successfully':'New Currency has been added successfully.');
                }else{
                    $this->session->set_flashdata('error',($id > 0)?'Unable to update Currency.':'Unable to add New Currency');
                }
            }
        }
        redirect('forex/currency_index');
    }

    public function currency_index(){
        $data['title']= "Forex";
        $data['currencies']=$this->crud->get_all('igc_forex_currency_detail');
        $this->load->view('header', $data);
        $this->load->view('forex/index',$data);
        $this->load->view('footer');
    }

    public function currency_delete($id){
        $result=$this->crud->delete($id,'currency_id','igc_forex_currency_detail');

        if($result){
            $this->session->set_flashdata('success','Currency Deleted Successfully');
        }else{
            $this->session->set_flashdata('success','Unable Deleted Currency');
        }
        redirect('forex/currency_index');
    }
