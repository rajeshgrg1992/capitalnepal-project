...
class Crud_model extends CI_Model
{
    ...
    public function get_where_pagination($table,$array,$start_point,$limit){
        ...
        $this->db->order_by("created",'DESC')->limit($limit,$start_point);
        $result = $this->db->get($table)->result_array();
        return $result;
    }
    ...
    public function get_where_desc($table,$array){
        if(count($array) > 0){
            foreach($array as $key => $value){
                $this->db->where("$key","$value");
            }
    ...
    public function get_where_desc1($table,$array){
        if(count($array) > 0){
            foreach($array as $key => $value){
                $this->db->where("$key","$value");
            }
        }
        $result = $this->db->order_by("created",'DESC')->get($table)->result_array();
        return $result;
    }
