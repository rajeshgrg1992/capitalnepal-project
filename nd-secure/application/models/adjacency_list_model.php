...
class Adjacency_list_model extends CI_Model
{
	...
	{
		return $this->db->where('content_id', $item_id)->get($this->tables['lists'])->row_array();
	}
	...
	{
		$return = $this->db->where('content_id', $item_id)->update($this->tables['lists'], $data);
		if ($return)
		{
	...
	{
		$this->db->where('content_id', $item_id)->delete($this->tables['lists']);
		if ($this->db->affected_rows())
		{
	...
	public function reorder($list)
	{
		...
		foreach ($list as $id => $parent_id)
		{
			...
			$this->db->where('content_id', $id)->set('parent_id', $p_id)->set('position', $position)->update($this->tables['lists']);
			$position++;
		}
