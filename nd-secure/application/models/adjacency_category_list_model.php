 ...
 * @since   Version 0.1
 */
class Adjacency_category_list_model extends CI_Model
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->database();
        $this->load->config('adjacency_category_list', TRUE);
        $this->lang->load('adjacency_category_list');

        $this->tables = $this->config->item('tables', 'adjacency_category_list');
        $this->flash  = $this->config->item('flash', 'adjacency_category_list');
    }
    ...
    {
        return $this->db->where('category_id', $item_id)->get($this->tables['lists'])->row_array();
    }
    ...
    {
        $return = $this->db->where('category_id', $item_id)->update($this->tables['lists'], $data);
        if ($return)
        {
    ...
    {
        $this->db->where('category_id', $item_id)->delete($this->tables['lists']);
        if ($this->db->affected_rows())
        {
    ...
    public function reorder($list)
    {
        ...
        {
            $p_id = ($parent_id == 'null')?"0" : $parent_id;
            $this->db->where('category_id', $id)->set('parent_id', $p_id)->set('position', $position)->update($this->tables['lists']);
            $position++;
        }
    ...
    public function delete_group($item_id)
    {
        ...
        if ($ar)
        {
            $this->db->where('group_id', $item_id)->delete($this->tables['eb_category']);
        }
