...
if ( ! function_exists('build_admin_tree'))
{
    ...
    function build_admin_tree(&$tree)
    {
        ...
        if ( ! empty($tree))
        {
            ...
            {
                $leaf['category_name'] = htmlspecialchars($leaf['category_name'], ENT_QUOTES, 'UTF-8');

                if (isset($leaf['children']) && ! empty($leaf['children']))
                {
                    $output .= '<li id="list_' . $leaf['category_id'] . '"><div><i class="icon-move"></i> ' . $leaf['category_name'] . '</div>';
                    $output .= '<ol>' . build_admin_tree($leaf['children']) . '</ol>';
                    $output .= '</li>';
                }
                else
                {
                    $output .= '<li id="list_' . $leaf['category_id'] . '"><div><i class="icon-move"></i> ' . $leaf['category_name'] . '</div></li>';
                }
            }
        }
...
if ( ! function_exists('build_tree'))
{
    ...
    function build_tree($group, $attributes = array(), &$tree = array())
    {
        ...
        {
            $CI =& get_instance();
            $CI->load->library('adjacency_category_list');
        ...
        if ( ! empty($tree))
        {
            foreach ($tree as &$leaf)
            {
                ...
                if (isset($leaf['children']) && ! empty($leaf['children']))
                {
                    $output .= $atts['start_tag'] . '<a href="' . $leaf['url'] . '">' . $leaf['category_name'] . '</a>';
                    $output .= $atts['sub_start_tag'] . build_tree($group, $attributes, $leaf['children']) . $atts['sub_end_tag'];
                    $output .= $atts['end_tag'];
                }
                else
                {
                    $output .= $atts['start_tag'] . '<a href="' . $leaf['url'] . '">' . $leaf['category_name'] . '</a>' . $atts['end_tag'];
                }
            }
        }
...
if ( ! function_exists('build_breadcrumb'))
{
    ...
    function build_breadcrumb($group, $item_id, $attributes = array(), &$tree = array(), &$output_tree = array())
    {
        ...
        if ( ! empty($tree))
        {
            foreach ($tree as &$leaf)
            {
                if ($item_id === (int) $leaf['category_id'])
                {
    ...
    function format_breadcrumb($array, $item_id, $attributes = array())
    {
        ...
        if ( ! empty($array) && is_array($array))
        {
            foreach ($array as &$item)
            {
                ...
                if ($item_id === (int) $item['category_id'])
                {
                    $output .= $atts['start_tag_active'] . $item['category_name'] . $atts['end_tag'];
                }
                else
                {
                    $output .= $atts['start_tag'] . '<a href="' . $item['url'] . '">' . $item['category_name'] . '</a>' . $atts['divider'] . $atts['end_tag'];
                }
            }
        }
    ...
    function build_tree_item($group, $item_id, $attributes = array(), &$tree = NULL, &$in_array = array())
    {
        ...
        if ( ! empty($tree))
        {
            foreach ($tree as &$leaf)
            {
                if ($item_id === (int) $leaf['category_id'])
                {
                    array_push($in_array, $leaf['category_id']);
    ...
    function format_tree(&$tree, &$in_array, $attributes = array())
    {
        ...
        if ( ! empty($tree))
        {
            foreach ($tree as &$leaf)
            {
                ...
                if (isset($leaf['children']) && ! empty($leaf['children']) && (in_array($leaf['category_id'], $in_array)))
                {
                    $output .= $atts['start_tag'] . '<a href="' . $leaf['url'] . '">' . $leaf['category_name'] . '</a>';
                ...
                {
                    $output .= $atts['start_tag'] . '<a href="' . $leaf['category_url'] . '">' . $leaf['category_name'] . '</a>' . $atts['end_tag'];
                }
            }
        }
...
if ( ! function_exists('parse_children'))
{
    ...
    function parse_children(&$query)
    {
        ...
        foreach ($tree as &$leaf)
        {
            ...
            {
                $tree[$leaf['parent_id']]['children'][] = &$tree[$leaf['category_id']];
            }

            if ( ! isset($tree[$leaf['category_id']]['children']))
            {
                $tree[$leaf['category_id']]['children'] = array();
            }

            if ( (int) $leaf['parent_id'] === 0)
            {
                $tree_array[] = &$tree[$leaf['category_id']];
            }
        }
