...
if ( ! function_exists('build_admin_tree'))
{
	...
	function build_admin_tree(&$tree)
	{
		...
		if ( ! empty($tree))
		{
			...
			{
				$leaf['content_page_title'] = htmlspecialchars($leaf['content_page_title'], ENT_QUOTES, 'UTF-8');

				if (isset($leaf['children']) && ! empty($leaf['children']))
				{
					$output .= '<li id="list_' . $leaf['content_id'] . '"><div><i class="icon-move"></i> ' . $leaf['content_page_title'] . '</div>';
					$output .= '<ol>' . build_admin_tree($leaf['children']) . '</ol>';
					$output .= '</li>';
				}
				else
				{
					$output .= '<li id="list_' . $leaf['content_id'] . '"><div><i class="icon-move"></i> ' . $leaf['content_page_title'] . '</div></li>';
				}
			}
		}
	...
	function build_tree($group, $attributes = array(), &$tree = array())
	{
		...
		if ( ! empty($tree))
		{
			foreach ($tree as &$leaf)
			{
				...
				if (isset($leaf['children']) && ! empty($leaf['children']))
				{
					$output .= $atts['start_tag'] . '<a href="' . $leaf['url'] . '">' . $leaf['content_page_title'] . '</a>';
					$output .= $atts['sub_start_tag'] . build_tree($group, $attributes, $leaf['children']) . $atts['sub_end_tag'];
					$output .= $atts['end_tag'];
				}
				else
				{
					$output .= $atts['start_tag'] . '<a href="' . $leaf['url'] . '">' . $leaf['content_page_title'] . '</a>' . $atts['end_tag'];
				}
			}
		}
...
if ( ! function_exists('build_breadcrumb'))
{
	...
	function build_breadcrumb($group, $item_id, $attributes = array(), &$tree = array(), &$output_tree = array())
	{
		...
		if ( ! empty($tree))
		{
			foreach ($tree as &$leaf)
			{
				if ($item_id === (int) $leaf['content_id'])
				{
...
if ( ! function_exists('format_breadcrumb'))
{
	...
	function format_breadcrumb($array, $item_id, $attributes = array())
	{
		...
		if ( ! empty($array) && is_array($array))
		{
			...
			{
				$item['content_page_title'] = htmlspecialchars($item['content_page_title'], ENT_QUOTES, 'UTF-8');

				if ($item_id === (int) $item['content_id'])
				{
					$output .= $atts['start_tag_active'] . $item['content_page_title'] . $atts['end_tag'];
				}
				else
				{
					$output .= $atts['start_tag'] . '<a href="' . $item['url'] . '">' . $item['content_page_title'] . '</a>' . $atts['divider'] . $atts['end_tag'];
				}
			}
		}
	...
	function build_tree_item($group, $item_id, $attributes = array(), &$tree = NULL, &$in_array = array())
	{
		...
		if ( ! empty($tree))
		{
			foreach ($tree as &$leaf)
			{
				if ($item_id === (int) $leaf['content_id'])
				{
					array_push($in_array, $leaf['content_id']);
	...
	function format_tree(&$tree, &$in_array, $attributes = array())
	{
		...
		if ( ! empty($tree))
		{
			foreach ($tree as &$leaf)
			{
				...
				if (isset($leaf['children']) && ! empty($leaf['children']) && (in_array($leaf['content_id'], $in_array)))
				{
					$output .= $atts['start_tag'] . '<a href="' . $leaf['url'] . '">' . $leaf['content_page_title'] . '</a>';
				...
				{
					$output .= $atts['start_tag'] . '<a href="' . $leaf['category_url'] . '">' . $leaf['content_page_title'] . '</a>' . $atts['end_tag'];
				}
			}
		}
...
if ( ! function_exists('parse_children'))
{
	...
	function parse_children(&$query)
	{
		...
		foreach ($tree as &$leaf)
		{
			...
			{
				$tree[$leaf['parent_id']]['children'][] = &$tree[$leaf['content_id']];
			}

			if ( ! isset($tree[$leaf['content_id']]['children']))
			{
				$tree[$leaf['content_id']]['children'] = array();
			}

			if ( (int) $leaf['parent_id'] === 0)
			{
				$tree_array[] = &$tree[$leaf['content_id']];
			}
		}
...
/*Edit and Delete tag for the menu*/
/*<span><a class="btn btn-primary btn-mini" href="' . site_url('al/edit/' . $leaf['content_id']) . '"><i class="icon-pencil icon-white"></i> Edit</a> <a class="btn btn-danger btn-mini delete" data-toggle="modal" data-type="item" data-href="' . site_url('al/delete/' . $leaf['content_id']) . '" data-name="' . $leaf['content_page_title'] . '" href="javascript:;"><i class="icon-trash icon-white"></i> Delete</a></span>
