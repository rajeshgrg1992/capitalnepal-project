...
class Adjacency_category_list
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('adjacency_category_list_model');
        $this->load->helper('adjacency_category_list');

        $this->max_levels = $this->config->item('max_levels', 'adjacency_category_list');
        $this->dropdown   = $this->config->item('dropdown', 'adjacency_category_list');
    }
    ...
    {
        if ( ! method_exists($this->adjacency_category_list_model, $method))
        {
            throw new Exception('Undefined method Adjacency_list::' . $method . '() called');
        }

        return call_user_func_array(array($this->adjacency_category_list_model, $method), $arguments);
    }
    ...
    public function get_all_for_dropdown($group_id = 1, $exclude = 0, $level = 0, &$tree = array())
    {
        ...
        if ( ! empty($tree))
        {
            ...
            {
                if ($exclude != (int) $leaf['category_id'])
                {
                    $output[$leaf['category_id']] = str_repeat($this->dropdown['space'], $level) . ' ' . htmlspecialchars($leaf['category_name'], ENT_QUOTES, 'UTF-8');
